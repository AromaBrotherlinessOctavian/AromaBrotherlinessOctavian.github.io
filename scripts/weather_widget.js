    ...
    .ready(function () {
        ...
        fetch(api_url, { signal })
            .then(response => {
                ...
                init_weather_widget(JSON.stringify(response));
            }).catch(e => {
                console.log(e);
                init_weather_widget(hardcoded_good_api_response);
                widget_display_error("Ошибка подключения к API! Будут использованы захардкоженные данные");
            });
    })
    ;
function init_weather_widget(json_text) {
    mount_weather_widget(parse_weather_data(JSON.parse(json_text)));
...
function parse_weather_data(api_response) {
